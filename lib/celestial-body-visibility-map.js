...
export function drawCelestialBodyVisibilityMap(
  ...
  let sunAltitude = astronomyJS.getAltitudeAzimuthCoordinatesForObject("Sun").altitude;
  ...
  if (altAzCoordinates.altitude < 0) {
    svg += drawCelestialBody(celestialBody, true, illuminatedFraction);
    ...
    let cx = 500;
    let cy = 520;
    let radiusFromCenter = radius * (1 - altAzCoordinates.azimuth / 90);
