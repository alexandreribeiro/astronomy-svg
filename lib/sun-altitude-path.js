...
export function drawSunAltitudePath(
  ...
  const results = intervals
    .map((date, i) => {
      ...
      const altitude = astronomyJS.getAltitudeAzimuthCoordinatesForObject(
        "Sun",
        date,
      ).altitude;
  ...
  svg += `
        ...
        <ellipse cx="${timeToPoints(minutesSinceMidnight, maxWidth)}" cy="${cy}" rx="${1.2 * maxWidth}" ry="125" fill="url(#twilight-glow-${sunAltitude})" clip-path="url(#bottom-half-clip)" />
        <rect x="0" y="500" width="${maxWidth}" height="500" fill="black" />
