export function drawCelestialBody(celestialBody, isGrayscale, illuminatedFraction) {
  let svg = isGrayscale ? startGrayscaleFilter() : "";
  if (celestialBody === "Sun") {
    svg += drawSun(isGrayscale);
  } else if (celestialBody === "Moon") {
    svg += drawMoon(illuminatedFraction);
...
function drawMoon(illuminatedFraction) {
  const clampedFraction = Math.max(0, Math.min(1, illuminatedFraction));

  if (clampedFraction === 0) {
    return `
      <!-- Moon base -->
      <circle cx="500" cy="278" r="139" fill="#5f6368" />
      <ellipse cx="455" cy="235" rx="18" ry="14" fill="#4a4e52" opacity="0.45" />
      <ellipse cx="545" cy="320" rx="16" ry="12" fill="#4a4e52" opacity="0.4" />
      <ellipse cx="500" cy="290" rx="12" ry="10" fill="#4a4e52" opacity="0.35" />
    `;
  }
  ...
  if (clampedFraction === 1) {
    return `
      ...
      <ellipse cx="455" cy="235" rx="18" ry="14" fill="#b8b4ab" opacity="0.35" />
      <ellipse cx="545" cy="320" rx="16" ry="12" fill="#b8b4ab" opacity="0.3" />
      <ellipse cx="500" cy="290" rx="12" ry="10" fill="#b8b4ab" opacity="0.25" />
    `;
  }

  const terminatorRadiusX = Math.max(0.001, Math.abs(2 * clampedFraction - 1) * 139);
  ...
  return `
    ...
    <!-- Surface detail -->
    <ellipse cx="455" cy="235" rx="18" ry="14" fill="#b8b4ab" opacity="0.35" clip-path="url(#moon-clip)" />
    <ellipse cx="545" cy="320" rx="16" ry="12" fill="#b8b4ab" opacity="0.3" clip-path="url(#moon-clip)" />
    <ellipse cx="500" cy="290" rx="12" ry="10" fill="#b8b4ab" opacity="0.25" clip-path="url(#moon-clip)" />
  `;
}
