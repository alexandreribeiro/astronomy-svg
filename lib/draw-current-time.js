...
export function drawCurrentTime(applicationContext, sizeInPixels) {
  ...
  svg += drawFirstRowText(
    now.toLocaleDateString(applicationContext.locale ?? undefined, {
      year: "numeric",
      month: "2-digit",
      day: "2-digit",
...
export function drawCurrentTimeWide(applicationContext, sizeInPixels) {
  let astronomyJS = applicationContext.astronomyJS;
  const now = astronomyJS.getDate();
  let svg = startRectangularSvg(sizeInPixels, "black");
  svg += `<g transform="translate(500,500) scale(2) translate(-500,-278)">`;
  svg += drawClock(applicationContext);
  svg += `</g>`;
  svg += drawText(
    now.toLocaleDateString(applicationContext.locale ?? undefined, {
...
function drawClock(applicationContext) {
  let astronomyJS = applicationContext.astronomyJS;
  const now = astronomyJS.getDate();
  let currentHour = now.toLocaleTimeString(
    applicationContext.locale ?? undefined,
    {
      hour: "2-digit",
      minute: "2-digit",
      hour12: false,
