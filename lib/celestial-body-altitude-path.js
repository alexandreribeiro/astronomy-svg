...
export function drawCelestialBodyAltitudePath(
  ...
  const celestialBodyResults = intervals
      .map((date) => {
  ...
  const celestialBodyCoordinates = astronomyJS.getAltitudeAzimuthCoordinatesForObject(celestialBody);

  const illuminatedFraction = astronomyJS.getIlluminatedFractionForObject(celestialBody);
  const isWaxing =
  ...
  const celestialBodyCy = mapRange(celestialBodyCoordinates.altitude, -90, 90, 950, 50).toFixed(0);
  ...
  svg += `
        ...
        <rect x="0" y="500" width="${maxWidth}" height="500" fill="black" />
        <polyline fill="none" stroke="url(#sun-line-gradient-${sunAltitude})" stroke-width="35" points="${celestialBodyResults}" clip-path="url(#top-half-clip)"/>
        <polyline fill="none" stroke="url(#sun-line-gradient-${sunAltitude})" stroke-width="35" points="${celestialBodyResults}" clip-path="url(#bottom-half-clip)"/>
        ...
        <g transform="translate(${azimuthToPoints(celestialBodyCoordinates.azimuth, maxWidth) - 500}, ${celestialBodyCy - 278})
